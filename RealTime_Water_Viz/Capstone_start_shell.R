...
server
variable <- c("Snowpack", "Ground Water", "Streamflow","Nutrient Levels")
...
ui <- fluidPage(
  tabsetPanel(
    ...
    tabPanel("Watershed 3",
             titlePanel("Watershed 3 Visualization"),
             selectInput("plot", label = "Select a plot type", choices = c("Histogram" , "Time Series" , "Reactive Graphs")),
             radioButtons("Variables", "Choose an Observed Variable", variable),
    ...
    ###Creates Tab and selection settings for Watershed 9 tab
    tabPanel("Both")
  )
)
